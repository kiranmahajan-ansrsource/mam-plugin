...
export class InsertPage extends LitElement {
  ...
  @state() private isLoadingAuth = true; // New state for auth loading
  ...
  async connectedCallback(): Promise<void> {
    ...
    // --- D2L OAuth Pre-check ---
    // Make sure fullImageUrl and name are present from URL params
    if (!this.image.fullImageUrl || !this.image.name) {
      ...
      this.goBack(); // Redirect back to the previous page
      return;
    }
    ...
    try {
      ...
      // Removed the Authorization header as it's not needed for this endpoint
      // and the backend checks for the d2lAccessToken cookie.
      ...
      // Explicitly check the 'authenticated' property from the response data
      // Also check if the status is not 200, which might indicate an issue even if data is present
      if (
        response.status !== 200 ||
        !response.data ||
        ...
        // Corrected the URL construction for the redirect
        window.location.href = `/oauth/login?returnTo=${encodeURIComponent(
          returnToUrl
        )}`;
        return; // Stop further execution until redirected back
      }
      ...
      // Optionally, redirect to a safe page or prompt for manual OAuth
      window.location.href = "/deeplink"; // Redirect to a default safe page
      return;
    } finally {
      this.isLoadingAuth = false;
    }
    // --- End D2L OAuth Pre-check ---
  }
