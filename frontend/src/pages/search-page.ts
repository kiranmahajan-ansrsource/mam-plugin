...
export class SearchPage extends LitElement {
  ...
  private async _triggerSearch() {
    ...
    try {
      ...
      const fallbackWorked = await this.fallbackErrorFunction(true); // true = full reset
      if (!fallbackWorked) {
  ...
  private async fallbackErrorFunction(reset: boolean): Promise<boolean> {
    try {
      const res = await axios.get("/api/search-images", {
        params: { query: this.searchTerm },
        headers: { Authorization: `Bearer ${this.ltik}` },
      });

      const items = res.data || []; // assuming it’s a flat array of image objects
      ...
      const mappedImages = items.map((item: any) => ({
        ...
        thumbnailUrl: item.mayoUrl, // or use a smaller version if available
        fullImageUrl: item.mayoUrl, // can be the same unless you separate preview vs full
        imageWidth: 600, // if not present, hardcode or extract from metadata
        imageHeight: 400,
        altText: item.altText || "",
        isDecorative: item.isDecorative || false,
        createDate: item.createdAt?.split("T")[0] || "", // optional formatting
      }));
